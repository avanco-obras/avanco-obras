...
describe('MeasurementsService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get(measurements_service_1.MeasurementsService);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('findByUnit', () => {
        it('should throw NotFoundException when unit does not exist', async () => {
            ...
            await expect(service.findByUnit(UNIT_ID)).rejects.toThrow(new common_1.NotFoundException(`Unidade com ID "${UNIT_ID}" não encontrada`));
        ...
        it('should return measurements array ordered desc when unit exists', async () => {
            ...
            expect(prisma.measurement.findMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('create with PERCENT method', () => {
        ...
        it('should throw NotFoundException when unit not found', async () => {
            prisma.unit.findUnique.mockResolvedValue(null);
            await expect(service.create(UNIT_ID, USER_ID, {
        ...
        it('should throw NotFoundException when activityType not found', async () => {
            ...
            await expect(service.create(UNIT_ID, USER_ID, {
                activityTypeId: ACTIVITY_TYPE_ID,
                percentComplete: 60,
            })).rejects.toThrow(new common_1.NotFoundException(`Tipo de atividade com ID "${ACTIVITY_TYPE_ID}" não encontrado`));
        });
        ...
        it('should call prisma.measurement.create with correct percentComplete', async () => {
            ...
            expect(prisma.measurement.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('update', () => {
        it('should throw NotFoundException when measurement not found', async () => {
            ...
            await expect(service.update(MEASUREMENT_ID, { executedQty: 60 })).rejects.toThrow(new common_1.NotFoundException(`Medição com ID "${MEASUREMENT_ID}" não encontrada`));
        ...
        it('should recalculate percentComplete for METRIC method based on new executedQty', async () => {
            ...
            expect(prisma.measurement.update).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('batchCreate', () => {
        it('should throw NotFoundException when unit not found', async () => {
            prisma.unit.findUnique.mockResolvedValue(null);
            await expect(service.batchCreate(UNIT_ID, USER_ID, {
                measurements: [{ activityTypeId: ACTIVITY_TYPE_ID, percentComplete: 50 }],
            })).rejects.toThrow(new common_1.NotFoundException(`Unidade com ID "${UNIT_ID}" não encontrada`));
